...
/* kolor z fragmentu rysunku: srgbClr (jawny), sysClr (jawny systemowy — np.
   czarny tekst/linia zapisana jako windowText) ALBO schemeClr (motyw +
   lumMod/lumOff/shade). Brak obsługi sysClr powodował, że jawnie czarna
   linia/tekst (bardzo częste w tych plikach: <a:sysClr val="windowText"
   lastClr="000000"/>) nie była w ogóle rozpoznana i kod cichcem sięgał po
   NIEWŁAŚCIWY kolor z odwołania do stylu (lnRef/fillRef) zamiast tego
   jawnego. */
function xlColor(xml) {
  if (!xml) return null;
  const m = xml.match(/<a:srgbClr\s+val="([0-9a-fA-F]{6})"/);
  if (m) return '#' + m[1].toLowerCase();
  const sy = xml.match(/<a:sysClr\s+val="[^"]*"\s+lastClr="([0-9a-fA-F]{6})"/);
...
function parseDrawingShapes(xml, dims, relMap = {}, mediaMap = {}) {
  ...
  while ((am = anchorRe.exec(xml)) !== null) {
    ...
    /* łączniki (strzałki wymiarowe/prowadnice) są z natury idealnie poziome
       LUB pionowe — jeden wymiar ich ramki wychodzi ~0px, to nie błąd, tylko
       geometria prostej linii. Ogólny filtr "obie strony >= 2px" (sensowny
       dla kształtów powierzchniowych/obrazów) całkowicie je odrzucał —
       znikały wszystkie poziome/pionowe strzałki wymiarowe z importu. */
    if (/<xdr:cxnSp[\s>]/.test(aXml)) { if (Math.max(w, h) < 2) continue; }
    else if (!w || !h || w < 2 || h < 2) continue;
